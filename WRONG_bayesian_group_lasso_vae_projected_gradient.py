...
* Gamma variables are parameterized directly and optimized with projected GD.

This model is WRONG in that it does not actually correspond to group lasso since
it optimizes over the gamma variables (thetas).
"""
...
thetas = Variable(torch.abs(torch.randn(num_groups, dim_z)), requires_grad=True)
